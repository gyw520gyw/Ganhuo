...
public class CustomCrashHandler implements UncaughtExceptionHandler {
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		...
		// 提示用户程序即将退出
		showToast(mContext, "很抱歉，程序遭遇异常，即将退出！gyw");
		try {
			thread.sleep(2000);
...
//		ExitAppUtils.getInstance().exit();
	...
	private String obtainExceptionInfo(Throwable throwable) {
		...
		LogUtil.d(mStringWriter.toString());
...
//		String downloadDir = FileUtil.getDownloadDir(mContext);
		if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
			...
			try {
				fileName = dir.getAbsolutePath() + File.separator + paserTime(System.currentTimeMillis()) + ".txt";
